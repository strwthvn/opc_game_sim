...
#### 6.1. Поток физики
- [x] Создать класс `PhysicsThread` в `include/simulation/PhysicsThread.h`
- [x] Использовать `std::thread` для запуска физического цикла
- [x] Реализовать fixed timestep 60 Hz (16.67 ms)
- [x] Добавить флаг `std::atomic<bool> running` для остановки потока

#### 6.2. Синхронизация между потоками
- [x] Использовать `std::mutex` для защиты доступа к `entt::registry`
- [x] Реализовать double buffering для TransformComponent:
  - Поток физики пишет в буфер A
  - Главный поток читает из буфера B
  - Swap буферов каждый кадр
- [x] Минимизировать время удержания мьютекса

#### 6.3. Управление жизненным циклом
- [x] Запуск потока в `GameState::initializeScene()` (физика управляется в GameState)
- [x] Остановка потока в `GameState::onExit()`
- [x] Обработка исключений в потоке физики (setExceptionHandler callback)
- [x] Логирование ошибок через spdlog (thread-safe)

#### 6.4. Тесты для многопоточности
- [x] Создать `tests/unit/test_physics_thread.cpp`
- [x] Тест: Поток запускается и останавливается корректно
- [x] Тест: Физика работает параллельно с главным потоком
- [x] Тест: Нет race conditions при доступе к registry (mutex protection)
- [x] Стресс-тест: 100 физических объектов в течение 1 секунды
- [x] Тест: Обработка исключений в потоке не крашит приложение
