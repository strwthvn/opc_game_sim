...
- [ ] **1.** Рефакторинг RenderSystem (разделить update/render)
- [ ] **2.** Добавить простую систему коллизий (тайловая)
- [ ] **3.** Реализовать SettingsState
...
- [ ] **1.1.1** Анализ текущего кода RenderSystem
  - [ ] Изучить `include/core/systems/RenderSystem.h`
  - [ ] Изучить `src/core/systems/RenderSystem.cpp`
  - [ ] Выявить, что делает `update()`, а что `render()`

- [ ] **1.1.2** Разделение ответственности
  - [ ] Перенести подготовку данных (кэширование, обновление спрайтов) в `update()`
  - [ ] Оставить только рендеринг (draw calls) в `render()`
  - [ ] Убрать вызов `render()` из `update()` если есть

- [ ] **1.1.3** Обновление GameState
  - [ ] Убедиться, что `SystemScheduler::update()` вызывает только `update()`
  - [ ] Добавить явный вызов `RenderSystem::render()` в `GameState::render()`

- [ ] **1.1.4** Тестирование
  - [ ] Запустить игру, проверить корректность рендеринга
  - [ ] Проверить производительность (FPS не должен упасть)
  - [ ] Протестировать с F6 (debug grid)
...
#### Задача 1.2: Добавление простой системы коллизий
...
- [ ] **1.2.1** Дизайн компонента
  - [ ] Создать `CollisionComponent` в `include/core/Components.h`:
    ```cpp
...
- [ ] **1.2.2** Создание CollisionSystem
  - [ ] Создать `include/core/systems/CollisionSystem.h`
  - [ ] Создать `src/core/systems/CollisionSystem.cpp`
  - [ ] Реализовать AABB проверку коллизий
  - [ ] Приоритет системы: **100** (между UpdateSystem и LifetimeSystem)

- [ ] **1.2.3** Интеграция с тайлами
  - [ ] Добавить метод в TileMapSystem для получения коллизий тайлов
  - [ ] Считывать свойство "collision" из TMX слоев
  - [ ] Создавать невидимые сущности с CollisionComponent для тайлов

- [ ] **1.2.4** Система событий коллизий
  - [ ] Добавить `std::function<void(entt::entity)> onCollisionEnter` в CollisionComponent
  - [ ] Генерировать события при начале/окончании коллизии

- [ ] **1.2.5** Регистрация в GameState
  - [ ] Добавить CollisionSystem в SystemScheduler
  - [ ] Обновить порядок выполнения систем в документации

- [ ] **1.2.6** Тестирование
  - [ ] Создать тестовую сцену с твердыми тайлами
  - [ ] Добавить объект с VelocityComponent и CollisionComponent
  - [ ] Проверить, что объект не проходит сквозь стены

**Критерии приемки:**
- ✅ AABB коллизии работают для статических объектов
- ✅ Твердые тайлы блокируют движение
- ✅ Триггеры детектируются без блокирования
- ✅ События `onCollisionEnter` вызываются корректно
...
- [ ] **1.3.1** Создание состояния
  - [ ] Создать `include/core/states/SettingsState.h`
  - [ ] Создать `src/core/states/SettingsState.cpp`
  - [ ] Наследоваться от `State`

- [ ] **1.3.2** Дизайн настроек (начальная версия)
  - [ ] Разрешение окна (список предустановок)
  - [ ] VSync вкл/выкл
  - [ ] Громкость (когда звук будет реализован - заглушка)
  - [ ] Кнопки управления (отображение, без переназначения пока)

- [ ] **1.3.3** UI разметка
  - [ ] Заголовок "Settings"
  - [ ] Список настроек (простой текст + кнопки)
  - [ ] Кнопка "Back" для возврата в меню
  - [ ] Навигация клавишами (вверх/вниз, Enter)

- [ ] **1.3.4** Сохранение настроек
  - [ ] Создать простой конфигурационный класс `Config`
  - [ ] Сохранение в `config.ini` или JSON (временно в памяти)
  - [ ] Применение настроек при выходе из SettingsState

- [ ] **1.3.5** Интеграция в MenuState
  - [ ] При выборе "Settings" → `m_stateManager->pushState(std::make_unique<SettingsState>(...))`
  - [ ] Убедиться, что возврат работает (popState)

- [ ] **1.3.6** Тестирование
  - [ ] Проверить навигацию в настройках
  - [ ] Проверить применение разрешения (требует рестарт пока)
  - [ ] Проверить возврат в меню
...
- [ ] **2.1.1** Добавить новые компоненты в `Components.h`
  ```cpp
...
- [ ] **2.1.2** Обновить документацию
  - [ ] Добавить описание компонентов в `ARCHITECTURE.md`
  - [ ] Добавить примеры использования

- [ ] **2.1.3** Интеграция с CollisionSystem
  - [ ] Использовать ColliderComponent вместо bounds в CollisionComponent
  - [ ] Поддержка разных форм коллайдеров (Box, Circle)

- [ ] **2.1.4** Тестирование
  - [ ] Создать сущности с разными формами коллайдеров
  - [ ] Проверить коллизии Box-Box, Box-Circle
...
- [ ] **2.2.1** Добавить EntityStateComponent
  ```cpp
...
- [ ] **2.2.2** Создать FSMSystem
  - [ ] Приоритет: **150** (после CollisionSystem)
  - [ ] Обновлять `timeInState`
  - [ ] Вызывать коллбеки при смене состояния

- [ ] **2.2.3** Пример использования
  - [ ] Создать тестовую сущность с FSM (например, "Lamp": off → on → broken)
  - [ ] Добавить переключение состояний по таймеру или событию

- [ ] **2.2.4** Документация
  - [ ] Добавить раздел в `ARCHITECTURE.md`
  - [ ] Примеры промышленных объектов с FSM
...
```
[ ] 1.1 Рефакторинг RenderSystem
    [ ] 1.1.1 Анализ текущего кода
    [ ] 1.1.2 Разделение update/render
    [ ] 1.1.3 Обновление GameState
    [ ] 1.1.4 Тестирование

[ ] 1.2 Система коллизий
    [ ] 1.2.1 Дизайн CollisionComponent
    [ ] 1.2.2 Создание CollisionSystem
    [ ] 1.2.3 Интеграция с тайлами
    [ ] 1.2.4 События коллизий
    [ ] 1.2.5 Регистрация в GameState
    [ ] 1.2.6 Тестирование

[ ] 1.3 SettingsState
    [ ] 1.3.1 Создание состояния
    [ ] 1.3.2 Дизайн настроек
    [ ] 1.3.3 UI разметка
    [ ] 1.3.4 Сохранение настроек
    [ ] 1.3.5 Интеграция в MenuState
    [ ] 1.3.6 Тестирование
