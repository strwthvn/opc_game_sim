...
void StateManager::render(sf::RenderWindow& window) {
    ...
    // Идем сверху вниз (от текущего состояния к нижним)
    // По умолчанию рендерим только верхнее состояние
    size_t startIndex = m_states.size() - 1;

    // Идем от верхнего (текущего) состояния вниз по стеку
    for (int i = static_cast<int>(m_states.size()) - 1; i > 0; --i) {
        if (m_states[i]->renderBelow()) {
            // Текущее состояние хочет рендерить состояния под собой
            // Проверяем следующее состояние ниже
            startIndex = i - 1;
        } else {
            // Текущее состояние НЕ хочет рендерить под собой
            // Начинаем рендеринг с него (не включая состояния ниже)
            startIndex = i;
            break;
        }
    }
